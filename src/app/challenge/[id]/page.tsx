...
export default function ChallengePage({ params }: { params: { id: string } }) {
  ...
  }, [router, params.id, challenge?.status]);
  ...
  useEffect(() => {
    if (challenge && solvedProblems.size > 0) {
      ...
      solvedProblems.forEach((solvedInfo, problemId) => {
        ...
        if (!existingSubmission) {
          ...
          const newSubmission: Submission = {
            ...
            submissionTime: solvedInfo.time.toISOString(),
            verdict: 'OK'
          };
    ...
    const calculateTimeLeft = () => {
      ...
      if (difference <= 0) {
        ...
        if (challenge.status === 'IN_PROGRESS') {
          fetchChallenge(localStorage.getItem('token') || '', params.id);
        }
        return;
      }
  ...
  }, [challenge, params.id, socketTimeLeft, serverTime, timeDiff]);
